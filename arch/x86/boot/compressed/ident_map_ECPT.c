...
/*
 * This code is used on x86_64 with ECPT turned on to create 
 * hash page table identity mappings on
 * demand by building up a new set of hash page tables (or appending to the
 * existing ones), and then switching over to them when ready.
 *
 * Copyright (C) 2015-2016  Yinghai Lu
 * Copyright (C)      2016  Kees Cook
 * Copyright (C)      2021  Siyuan Chai
 */
...
static void add_identity_map(unsigned long start, unsigned long end)
{
	...
	start = round_down(start, BOOT_PAGE_SIZE);
	end = round_up(end, BOOT_PAGE_SIZE);
	if (start >= end)
		return;
...
void initialize_identity_maps(void *rmode)
{
     ...
     *  Right now, BOOT_PGT_SIZE = BOOT_INIT_PGT_SIZE
     * */
