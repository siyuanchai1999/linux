...
#ifndef __HAVE_ARCH_FREE_PGD_RANGE
/**
 * Note: free_pgd_range function should be considered as part of interface 
 * that we redo the implemenation. The reason is that there's no mapping 
 * from VA to pgtable page in ECPT.
 * Here, we substiute p*d_offset, p*d_none_or_clear_bad and pgtable iterator
 * with generalized interface to make it consistent with other functions in the file.
 * Again, free_pgd_range should be part of the interface.
 */
				...
				unsigned long floor, unsigned long ceiling)
{
	...
	pmd = pmd_offset_map_with_mm(tlb->mm, pud, addr);
			...
			unsigned long floor, unsigned long ceiling)
{
	...
	do {
		...
		if (pgd_next_level_not_accessible(src_pgd))
			continue;
...
again:
	...
	do {
		...
		&& (dst_pte = ptep_get_next(dst_mm, dst_pte, addr - PAGE_SIZE),
			src_pte = ptep_get_next(src_mm, src_pte, addr - PAGE_SIZE)));
	       ...
	       unsigned long end)
{
	...
	src_pmd = pmd_offset_map_with_mm(src_mm, src_pud, addr);
	/* original code */
	...
	do {
		...
		if (pmd_next_level_not_accessible(src_pmd))
			continue;
	...
	while(next != end ? 
		(dst_pmd = pmdp_get_next(dst_mm, dst_pmd, addr),
		 	src_pmd = pmdp_get_next(src_mm, src_pmd, addr), addr = next) : 
		(addr = next, 0));
	/* original code */
	       ...
	       unsigned long end)
{
	...
	do {
		...
		if (pud_next_level_not_accessible(src_pud))
			continue;
		...
		if (copy_pmd_range(dst_vma, src_vma, dst_pud, src_pud,
			...
			(dst_pud = pudp_get_next(dst_mm, dst_pud, addr),
				src_pud = pudp_get_next(dst_mm, src_pud, addr), addr = next) : 
			(addr = next, 0));
	       ...
	       unsigned long end)
{
	...
	do {
		...
		if (p4d_next_level_not_accessible(src_p4d))
			continue;
		/* original code */
		...
		if (copy_pud_range(dst_vma, src_vma, dst_p4d, src_p4d,
			...
			(dst_p4d = p4dp_get_next(dst_mm, dst_p4d, addr),
				src_p4d = p4dp_get_next(src_vma->vm_mm, src_p4d, addr), addr = next) : 
			(addr = next, 0));
	/* original code */
...
copy_page_range(struct vm_area_struct *dst_vma, struct vm_area_struct *src_vma)
{
	...
	pr_info_verbose("dst_vma at %llx src_vma at %llx addr=%lx end=%lx\n", 
			(uint64_t) dst_vma, (uint64_t) src_vma, addr, end);
	/*
	...
	do {
		...
		(dst_pgd = pgdp_get_next(dst_vma->vm_mm, dst_pgd, addr),
		 	src_pgd = pgdp_get_next(src_vma->vm_mm, src_pgd, addr), addr = next) : 
		(addr = next, 0));
				 ...
				 struct vm_fault *vmf)
{
	...
	pr_info_verbose("kaddr=%llx uaddr=%llx\n", (uint64_t) kaddr, (uint64_t) uaddr);
	/*
...
static vm_fault_t wp_page_copy(struct vm_fault *vmf)
{
	...
	int page_copied = 0;
	struct mmu_notifier_range range;
	pr_info_verbose("vmf->pte at %llx =%lx", (uint64_t) vmf->pte, vmf->pte->pte);
